...
const TestNavBar = () => {
    ...
    const NavItem = SimpleNavItem(DefaultNavItemStyles);
...
describe("SimpleNavItem with TranslationProvider", () => {
    it("renders nav items with translation", () => {
        ...
        expect(screen.getByText("home-trans")).toBeInTheDocument();
        expect(screen.getByText("profile-trans")).toBeInTheDocument();
        expect(screen.getByText("settings-trans")).toBeInTheDocument();
    });

    it("applies selected style to the selected nav item", () => {
        render(<TestNavBar />);

        const homeButton = screen.getByText("home-trans");
        const profileButton = screen.getByText("profile-trans");
    ...
    it("changes selection on click", async () => {
        ...
        await waitFor(() => {
            const updatedProfileButton = screen.getByText("profile-trans");
            const updatedHomeButton = screen.getByText("home-trans");
