...
export type LoadingOrErrorFn = (props: LoadingOrErrorProps) => React.ReactElement;
...
export function LoadingOr<Input, Output>({input, kleisli, loading = defaultLoading, error = defaultError, children, onUnmount = () => {}}: LoadingOrProps<Input, Output>): React.ReactElement {
    ...
    if (hasError(state)) return error!({error: state.error as string});
