...
//Here we have an api that needs you to do an initial call to set up the world before the query
//A good example is elastic search where we need to do a PIT (point in time) call to get a cursor before we can do a search
//The page1 call is the call that sets up the world

type PagingByApi<Context, From,Paging> = DomainCaller<Context, From, Paging>

export type SearchByApiPagingTypeClass<Context, Data, Paging> = SearchTypeClass<Paging> & DataAggregatesAndPagingResultMaker<Context, Data, Paging> & DomainCaller<Context, QueryDatasourceAndPaging<Paging>, DataAggregatesAndPaging<Data, Paging>> & {
    type: 'api'
